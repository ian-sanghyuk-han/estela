...
**아니다.** 식품위생법 시행령은 식품접객업을 여섯으로 나누고, 그 데이터셋은 그중 하나다.

| 업종 | 무엇이 들어가나 | data.go.kr |
|---|---|---|
| 일반음식점 | 밥집. 식사와 함께 술도 판다 | `15045016` (표준판 `15096283`) |
| **휴게음식점** | **카페 · 분식 · 패스트푸드 · 아이스크림** (술 못 판다) | `15006730` |
| **제과점영업** | **빵집 · 케이크** | `15044973` |
| 단란주점영업 | 술집 (노래 가능) | `15045017` |
| 유흥주점영업 | 룸살롱·클럽 등 유흥종사자를 두는 곳 | `15045018` |
| 위탁급식영업 | 구내식당 위탁 — **뺀다** | `15044975` |

프랑스에서 겪은 것과 똑같은 함정이다. Alim'confiance의 다섯 갈래 중 `Restaurants`만
쓰다가 빵집 7,764곳과 정육 2,998곳을 통째로 놓치고 있었다. **명단을 받을 때는 그 나라가
업종을 어떻게 쪼개 놓았는지부터 봐야 한다** — 우리가 아는 '식당'의 경계와 다르다.

가져올 것은 다섯: 일반음식점 · 휴게음식점 · 제과점영업 · 단란주점 · 유흥주점.
위탁급식은 뺀다 — 영국에서 학교와 병원을, 프랑스에서 급식을 뺀 것과 같은 이유다.

### LOCALDATA는 죽은 게 아니라 이사 갔다

**LOCALDATA(localdata.go.kr)는 2026년 4월 16일 0시부로 폐쇄되었다.** 인허가 195종과
생활편의정보 14종이 전부 공공데이터포털(data.go.kr)로 옮겨졌다. 접속이 안 되는 것은
고장이 아니라 그 때문이다 — DNS는 152.99.104.122을 찾아 주지만 443 포트가 닫혀 있다.

그래서 "열쇠 하나로 196개 업종"이라는 장점은 **없어졌다.** 지금은 data.go.kr에서
업종마다 따로 활용신청을 해야 한다. 다만 행정안전부가 권하는 **방식은 그대로 살아 있고,
그게 자동 갱신의 정답이다** — *전체는 파일로 한 번 받고, 그 뒤로는 API로 변동분만 받는다.*
매달 250만 건을 다시 받는 대신 새로 열고 닫은 몇천 곳만 받으면 된다.

### 받을 것 — 파일과 API가 짝지어 있다

| 업종 | 파일(첫 수확) | API(변동분) |
|---|---|---|
| 일반음식점 | `15045016` | `15154916` |
| 휴게음식점 (카페·분식) | `15006730` | `15154921` |
| 제과점영업 (빵집) | `15044973` | `15155252` |
| 단란주점영업 | `15045017` | `15154883` |
| 유흥주점영업 | `15045018` | `15154890` |
| ~~위탁급식영업~~ | ~~`15044975`~~ | ~~`15155159`~~ (구내식당 — 뺀다) |

주소는 `https://www.data.go.kr/data/<번호>/fileData.do` 와 `.../openapi.do`.

## 3단계 — 막혔던 나라가 열렸다 (2026-08-28)

**488,762곳 → 1,279,871곳. 나라 7개 → 22개.**

Tier D에 "막힘"으로 적어 두었던 나라들을 **Overture Maps Foundation**으로 다시 두드렸다.
`2026-08-19.0` 판, `taxonomy.hierarchy[1] = 'food_and_drink'`, `confidence ≥ 0.2`.
S3의 GeoParquet을 DuckDB로 읽으면 상자 하나가 6~40초고 내려받는 것은 몇 MB뿐이다.

| | 곳 | | 곳 |
|---|---|---|---|
| 방콕 | **135,773** | 멕시코시티 | 56,136 |
| 도쿄 23구 | **135,407** | 타이베이 | 53,088 |
| 서울 | **76,260** | 자카르타 | 50,695 |
| 호찌민 | 65,572 | 하노이 | 41,333 |
| 쿠알라룸푸르 | 35,128 | 이스탄불 | 31,935 |
| 싱가포르 (전국) | 29,116 | 베를린 | 17,649 |
| 뭄바이 | 17,339 | 델리 | 15,607 |
| 바르셀로나 | 15,847 | 암스테르담 | 7,561 |
| 스톡홀름 | 6,663 | | |

### 재 보기 전에는 옮기지 않는다 — 대조군

숫자가 크다고 좋은 자료는 아니다. 그래서 **우리가 이미 믿는 명부가 있는 도시**를
같은 상자로 두 번 셌다.

| | 우리 명부 | Overture | 비 |
|---|---|---|---|
| 런던 | 44,675 | 55,665 | 125% |
| 뉴욕 | 31,632 | 59,605 | 188% |
| 홍콩 | 17,092 | 36,861 | 216% |
| 파리 | 6,654 | 29,092 | 437% |

Overture가 더 많다. 다만 **더 많다고 더 옳은 것은 아니다** — 우리 명부가 일부러 뺀
갈래(마트·급식)가 저쪽에는 들어 있고, 파리가 437%인 것은 Alim'confiance가 *점검받은 곳만*
담기 때문이다(이미 적어 둔 한계다). 그래도 이 비율이 말해 주는 것은 분명하다:
**Overture는 정부 명부가 없는 곳에서 빈손으로 돌아오지 않는다.**
...
| 주소 있음 | 98% | 99% |
| 겹침 | 0.0% | 0.0% |
| 평균 confidence | 0.71 | 0.64 |

Overture 문서는 "중복이 많고 쓰레기 비율이 높다"고 스스로 경고하는데,
**food_and_drink 갈래에서는 그렇지 않았다** — 이미 합쳐 놓았기 때문이다.

**현지 문자 이름 비율**: 타이베이 87% · 서울 83% · 도쿄 79% · 방콕 66%.
서 있는 사람이 실제로 치는 글자로 찾을 수 있다는 뜻이다.
서울에서 `순대`를 치면 인하찹쌀순대·담소사골순대·유명순대국이 나온다.

### 격자를 출처마다 다르게 (중요)

0.25° 칸 하나가 방콕에서는 **2.7 MB**가 되었다. 검색 한 번에 그걸 다 내려받게 된다.
그래서 manifest를 **version 2**로 올리고 **출처마다 제 격자 크기**를 갖게 했다 —
정부 명부는 0.25° 그대로, Overture 도시들은 **0.02°**. 칸 평균이 3~64 KB로 내려왔다.

그리고 화면에 걸친 칸을 셀 때 **칸 수가 아니라 줄 수로** 막는다(6만 줄). 그게 실제 무게다.
서비스 지역 칠하기도 촘촘한 출처는 **도시 상자 하나**로 그린다 — 바늘구멍 수백 개는
세계 배율에서 안 보이고, 우리가 뒤진 상자가 곧 그 상자이므로 뭉뚱그리는 것도 아니다.

### 이것이 대체하지 않는 것

**서울 76,260곳은 한국 210만 곳의 3.6%다.** Overture는 구멍을 메우지 바닥을 깔지 않는다.
`data.go.kr` 열쇠는 여전히 이 프로젝트에서 가장 큰 한 방이다.

## 2단계 — 도시를 넓힌다 (2026-08-28)

**368,951곳 → 488,762곳. 출처 9개 → 19개.**

### 프랑스 — 이미 받아 둔 파일 안에 내내 있었다

Alim'confiance는 다섯 갈래인데 우리는 하나만 쓰고 있었다. `Métiers de bouche` 14,395곳을
더 가져왔다 — **빵집·제과 7,764 · 정육·샤퀴트리 2,998 · 반찬 841 · 치즈 757 · 생선 660 ·
초콜릿 · 아이스크림.** 미식 지구본에 빵집은 곁가지가 아니다. 파리의 아침은 그게 전부다.

`Restauration collective`(급식)와 `Supermarchés`(마트)는 계속 뺀다 — 영국에서 학교와
병원을 뺀 것과 같은 이유다. `Producteurs fermiers`(농가)도 뺀다: 음식이 나오는 곳이지
먹으러 가는 곳이 아니다. 레스토랑도 32,507 → 32,745로 갱신했다.

### 미국 — 손으로 하던 것을 기계로

뉴욕·시카고·보스턴은 각각 손으로 짰다. 그 속도로는 하루에 한 도시다.
그런데 영어권 보건당국은 대개 **Socrata**라는 같은 소프트웨어를 쓰고 API 모양도 같다.
그래서 넷째 도시부터는 스크립트가 아니라 **표의 한 줄**이 되게 만들었다(`socrata.py`).

Socrata 검색으로 후보 135개를 모아 **칸을 하나씩 열어 봤고**(제목을 믿지 않는다),
좌표가 실제로 들어 있는 것만 남겼다. 9곳이 새로 붙었다:

| 새로 붙은 곳 | 곳 |
|---|---|
| 펜실베이니아 (필라델피아·피츠버그 포함, 주 전체) | **54,074** |
| 뉴욕주 (뉴욕시 외) | **17,637** |
| 댈러스 | 8,693 |
| 산호세·산타클라라 | 9,684 |
| 신시내티 | 6,152 |
| 몽고메리카운티 MD (워싱턴 근교) | 4,069 |
| 캔자스시티 | 2,803 |
| 케임브리지 MA | 1,523 |
| 볼더카운티 CO | 543 |

**좌표가 없어 미룬 곳:** 로스앤젤레스 · 시애틀·킹카운티 · 오스틴 · 에드먼턴 ·
노바스코샤. 주소는 있으니 홍콩처럼 지오코딩하면 살아난다.
애틀랜타(풀턴카운티)는 좌표 칸이 **있는데 33,124행이 전부 비어 있었다**.

### 이번에 배운 것 — 남의 지오코딩을 믿지 마라

포털은 저마다 제 주소를 좌표로 바꾸는데, 저마다 틀린다.
**댈러스는 식당의 21.8%를 댈러스 밖에 찍었다 — 하와이와 알래스카까지.**
뉴욕주는 경도 −778을 냈고, 산호세는 한 쌍을 뒤집어 터키에 식당을 하나 세웠다.

이건 **들여다보기 전에는 안 보이고, 실어 버리면 다시 안 보인다** — 지구 반대편의
잘못된 점은 거기 가 본 적 없는 사람에게는 맞는 점과 똑같이 생겼다.
그래서 출처마다 **제 관할 상자**를 선언하게 하고 밖으로 나간 것은 버린다(`clean.py`).
상자는 넉넉하게 두른다: 잡으려는 건 시간대를 넘어가는 오류지 교외가 아니다.

같은 이유로 **같은 이름 + 같은 주소는 한 집으로** 접었다. 점검 기록은 한 집에 여러 줄이고,
좌표가 조금씩 흔들려서 좌표만으로는 안 접힌다. 댈러스가 11,037 → 8,693으로 줄었다.
그리고 댈러스의 `type` 칸은 업종이 아니라 **점검 종류**(Routine)여서 업종 칸을 비웠다.
...
## 막힌 나라는 구글이 어떻게 하고 있는가 (조사 2026-08-27)

싱가포르·태국·베트남·인도네시아처럼 정부 명부가 없는 나라에도 구글 지도에는 식당이
빼곡하다. 어디서 오는가를 알아보니, **우리가 못 하는 셋과 우리가 할 수 있는 하나**로 갈렸다.

### 우리가 따라 할 수 없는 셋

1. **업주가 제 발로 온다 — Google Business Profile.** 가장 큰 몫이고 가장 따라 하기 어렵다.
   구글에 실리는 것이 장사에 값이 되니까 업주가 직접 등록하고 직접 고친다.
   이건 돈으로 살 수 있는 것이 아니라 **이미 기본값이라서 생기는 해자**다.
2. **스트리트뷰 + 글자 읽기.** 차와 트레커가 87개국에서 **1,700억 장**을 찍었고,
   구글은 거기서 간판·주소·상호를 읽어낸다. 말 그대로 세상의 모든 가게 앞을 촬영한 것이다.
3. **로컬 가이드.** 수백만 명이 사진과 정보를 고쳐 넣는다.

여기에 **1,000개가 넘는 제3자 데이터 공급처**와 웹 크롤링, 그리고 있는 곳에서는
정부·공공 기록(사업자 면허)을 함께 쓴다 — 마지막 문은 우리가 쓰는 문과 같다.

> 그러니 싱가포르의 구글 명단은 싱가포르 정부에서 온 것이 **아니다.**
> 업주와 스트리트뷰와 웹에서 온 것이다.

### 우리가 쓸 수 있는 하나 — **Overture Maps Foundation**

메타·마이크로소프트·아마존·톰톰이 만든 재단이 POI를 통째로 공개한다.

| | |
|---|---|
| 규모 | **약 7,400만 곳** (2026-08 기준). 메타가 약 5,800만으로 최대 |
| 출처 | Meta · Microsoft · Foursquare · PinMeTo · AllThePlaces · BrightQuery |
| 라이선스 | **CDLA Permissive 2.0 / Apache 2.0** — 출처에 따라 다름 |
| OSM 포함 | **아니다.** 그래서 ODbL 전염이 없다 |
| 스키마 | 이름(별칭 포함) · **약 2,300개 분류 체계** · 주소 · 전화 · 웹 · **confidence(존재 확률 0~1)** · GERS 영구 ID |
| 받는 법 | AWS S3에 GeoParquet. 나라·분류로 잘라 받을 수 있다 |

**핵심은 이것이다.** 메타의 5,800만 곳은 **페이스북 페이지** — 즉 구글과 똑같이
*업주가 제 발로 등록한* 자료다. 구글은 그걸 해자로 쥐고 있고, **메타는 그걸 공개한다.**
우리가 못 만드는 해자를 남이 나눠 주는 셈이다.

**정직한 한계 (그들 스스로 적어 둔 것):** 중복이 있고, 쓰레기 비율이 높고,
속성이 많이 비어 있다. `confidence` 점수로 거를 수 있지만 중복과 결측은 그것으로 안 풀린다.
그래서 **정부 명부를 대체하지는 못한다** — 정부 명부가 없는 곳을 메우는 용도다.

### 이것이 바꾸는 것

Tier D(막힘)로 적어 둔 나라들 — 싱가포르·태국·베트남·말레이시아·인도네시아·인도,
그리고 유럽에서 기업등기가 막힌 독일·네덜란드·스웨덴 — 이 **다시 열릴 수 있다.**
다만 아직 **재어 보지 않았다.** 다음 할 일은 싱가포르·방콕·도쿄를 Overture로 뽑아서
아는 거리 하나와 대조해 보는 것이다. 숫자를 보기 전에는 Tier를 옮기지 않는다.

*(2026-08-24 싱가포르에서 저지른 잘못을 되풀이하지 않기 위해 — 제목만 보고 믿지 않는다.)*
